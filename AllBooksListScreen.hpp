...
public:
    AllBooksListScreen(sf::Font& font, LibrarySystem* lib) : libSystem(lib) {
        ...
        titleText.setFont(font); titleText.setString("TAT CA SACH"); titleText.setCharacterSize(30); titleText.setFillColor(Theme::TextDark); titleText.setPosition(280, 30);
    ...
    void loadBooks(sf::Font& font) {
        for (auto card : allBookCards) delete card; allBookCards.clear(); if (!libSystem) return;
    ...
    void update(sf::Vector2f mousePos) { sidebar->update(mousePos); float scrollOffset = scrollView->getScrollOffset(); if (mousePos.y > VIEW_Y && mousePos.x > 250) { sf::Vector2f relativeMouse(mousePos.x - VIEW_X, mousePos.y - VIEW_Y + scrollOffset); for (auto card : allBookCards) card->update(relativeMouse, 0, 0); } else { for (auto card : allBookCards) card->update(sf::Vector2f(-1000, -1000), 0, 0); } }
    void handleScrollEvent(sf::Event& event, sf::Vector2f mousePos) { scrollView->handleScroll(event, mousePos); }
    std::string handleCardClick(sf::Vector2f mousePos) { float scrollOffset = scrollView->getScrollOffset(); if (mousePos.y > VIEW_Y && mousePos.x > 250) { sf::Vector2f relativeMouse(mousePos.x - VIEW_X, mousePos.y - VIEW_Y + scrollOffset); for (auto card : allBookCards) if (card->isClicked(relativeMouse)) return card->getBookId(); } return ""; }
    ...
    void render(sf::RenderWindow& window) { window.setView(window.getDefaultView()); window.draw(background); sidebar->draw(window); window.draw(titleText); sf::View gridView = window.getDefaultView(); gridView.setViewport(sf::FloatRect(VIEW_X/1300.f, VIEW_Y/720.f, VIEW_W/1300.f, VIEW_H/720.f)); gridView.setSize(VIEW_W, VIEW_H); gridView.setCenter(VIEW_W/2.f, VIEW_H/2.f + scrollView->getScrollOffset()); window.setView(gridView); for (auto card : allBookCards) card->draw(window); window.setView(window.getDefaultView()); }
};
#endif