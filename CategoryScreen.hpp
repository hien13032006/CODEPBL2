...
private:
    ...
    std::vector<CategoryBox*> catBoxes; 
    ...
    const float VIEW_Y = 120.0f;
    const float VIEW_W = 990.0f;
    const float VIEW_H = 580.0f;
...
public:
    ...
    ~CategoryScreen() { 
        ...
        delete backButton; 
        delete scrollView; 
        for (auto box : catBoxes) delete box; 
    ...
    void loadCategories(sf::Font& font) {
        ...
        while (current) { 
            catCounts[current->data->getTheLoai()]++; 
            current = current->next; 
        }

        std::vector<sf::Color> colors = {
            sf::Color(173, 216, 230), sf::Color(255, 182, 193), 
            sf::Color(152, 251, 152), sf::Color(255, 228, 181), 
            sf::Color(221, 160, 221)
        };
        
        float startX = 20; float startY = 20; 
        float boxW = 280; float boxH = 180; 
        float gapX = 320; float gapY = 220;
        int col = 0; int row = 0; int idx = 0;

        for (const auto& pair : catCounts) {
            CategoryBox* box = new CategoryBox();
            box->box.setSize({boxW, boxH});
            box->box.setCornerRadius(15.0f);
            box->box.setPosition(startX + col*gapX, startY + row*gapY);
            box->box.setFillColor(colors[idx % 5]);
            
            box->name.setFont(font);
            box->name.setString(pair.first);
            box->name.setCharacterSize(24);
            box->name.setFillColor(Theme::TextDark);
            sf::FloatRect b = box->name.getLocalBounds();
            box->name.setOrigin(b.width/2, b.height/2);
            box->name.setPosition(box->box.getPosition().x + boxW/2, box->box.getPosition().y + boxH/2 - 10);

            box->count.setFont(font);
            box->count.setString(std::to_string(pair.second) + " dau sach");
            box->count.setCharacterSize(16);
            box->count.setFillColor(sf::Color(80, 80, 80));
            sf::FloatRect c = box->count.getLocalBounds();
            box->count.setOrigin(c.width/2, c.height/2);
            box->count.setPosition(box->box.getPosition().x + boxW/2, box->box.getPosition().y + boxH/2 + 25);
            
            box->isHovered = false;
            catBoxes.push_back(box);
            
            idx++; col++; if (col == 3) { col = 0; row++; }
        }
        
        float contentBottom = startY + ((catBoxes.size() + 2) / 3) * gapY + 950.0f;
        scrollView->setMaxScroll(std::max(0.0f, contentBottom - VIEW_H)); 
        scrollView->reset();
    }
    ...
    void loadBooksOfCategory(const std::string& category, sf::Font& font) {
        ...
        std::vector<sf::Color> colors = { 
            sf::Color(255, 182, 193), sf::Color(176, 224, 230), 
            sf::Color(221, 160, 221), sf::Color(240, 230, 140), 
            sf::Color(152, 251, 152) 
        };
        
        NodeBook* current = libSystem->getDanhSachSach();
        float startX = 10; float startY = 20; 
        float cardW = 170; float cardH = 250; 
    ...
    void update(sf::Vector2f mousePos) {
        ...
        if (!selectedCategory.empty()) backButton->update(mousePos);
        ...
        if (mousePos.y > VIEW_Y && mousePos.x > 250) {
            ...
            if (selectedCategory.empty()) {
                for (auto box : catBoxes) {
                    if(box->box.getGlobalBounds().contains(rel)) {
                        if(!box->isHovered) { 
                            box->isHovered = true; 
                            box->box.setOutlineThickness(3); 
                            box->box.setOutlineColor(Theme::Secondary); 
                        }
                    } else {
                        if(box->isHovered) { 
                            box->isHovered = false; 
                            box->box.setOutlineThickness(0); 
                        }
                    }
                }
            } else {
                for (auto card : categoryBookCards) card->update(rel, 0, 0);
            }
        }
    }
    ...
    bool handleCategoryButtonClick(sf::Vector2f mousePos, sf::Font& font) {
        ...
        if (mousePos.y > VIEW_Y && mousePos.x > 250) {
            sf::Vector2f rel(mousePos.x - VIEW_X, mousePos.y - VIEW_Y + scrollOffset);
            // [FIX] Đảm bảo rel được định nghĩa trong phạm vi này để dùng trong vòng lặp
            for (auto box : catBoxes) {
                if (box->box.getGlobalBounds().contains(rel)) {
                    loadBooksOfCategory(box->name.getString(), font);
                    return true;
                }
            }
        }
    ...
    std::string handleCardClick(sf::Vector2f mousePos) {
        ...
        if (mousePos.y > VIEW_Y && mousePos.x > 250) {
            sf::Vector2f rel(mousePos.x - VIEW_X, mousePos.y - VIEW_Y + scrollOffset);
            // [FIX] Đảm bảo rel được định nghĩa trong phạm vi này
            for (auto card : categoryBookCards) {
                if (card->isClicked(rel)) return card->getBookId();
            }
        }
        return "";
    }
    ...
    void render(sf::RenderWindow& window) {
        ...
        view.setViewport(sf::FloatRect(VIEW_X/1300.f, VIEW_Y/720.f, VIEW_W/1300.f, VIEW_H/720.f));
        view.setSize(VIEW_W, VIEW_H);
        view.setCenter(VIEW_W/2.f, VIEW_H/2.f + scrollView->getScrollOffset());
        ...
        if (selectedCategory.empty()) { 
            for (auto box : catBoxes) { 
                window.draw(box->box); 
                window.draw(box->name); 
                window.draw(box->count); 
            }
        } else { 
            for (auto card : categoryBookCards) card->draw(window); 
        }
