...
private:
    ...
    Button *returnButton, *cancelButton, *yesButton, *noButton;
    bool shaking = false;
...
public:
    ReturnBookScreen(sf::Font& font, Modal* modalRef) : modal(modalRef) {
        ...
        panel.setPosition(300, 135);
        titleText.setFont(font); 
        titleText.setString("TRA SACH"); 
        titleText.setCharacterSize(32);
        titleText.setFillColor(Theme::Primary);
        titleText.setPosition(580, 170);
        maSachField = new InputField({350, 240}, {600, 55}, "Nhap ma sach can tra...", font);
        returnMsg.setFont(font);
        returnMsg.setCharacterSize(18); 
        returnMsg.setFillColor(Theme::Error);
        returnMsg.setPosition(350, 310);
        returnButton = new Button({370, 480}, {200, 55}, "Tra Sach", font, 0, Theme::Secondary);
        cancelButton = new Button({730, 480}, {200, 55}, "Huy", font, 0, sf::Color(150, 150, 150));
        ratingPanel.setSize({500, 300}); 
        ratingPanel.setCornerRadius(15.0f);
        ratingPanel.setFillColor(sf::Color::White); 
        ratingPanel.setOutlineThickness(2);
        ratingPanel.setOutlineColor(Theme::Success);
        ratingPanel.setPosition(400, 210);
        askRatingText.setFont(font); 
        askRatingText.setString("Tra sach thanh cong!\nBan co muon danh gia khong?");
        askRatingText.setCharacterSize(22);
        askRatingText.setFillColor(Theme::TextDark); 
        askRatingText.setPosition(440, 260);
        yesButton = new Button({450, 410}, {180, 50}, "Co, Luon!", font, 3, Theme::Success);
        noButton = new Button({670, 410}, {180, 50}, "De sau", font, 4, sf::Color(150, 150, 150));
    }
    ~ReturnBookScreen() { 
        delete returnButton;
        delete cancelButton; 
    ...
    bool validate() {
        if (getMaSachInput().empty()) { 
            setReturnMessage("Vui long nhap ma sach!");
            startShake(); 
            return false; 
        } 
        return true; 
    ...
    void update(sf::Vector2f mousePos) { 
        if (!askRatingVisible) { 
            ...
            if (shaking) { 
                shakeTime += 0.02f; 
                float offset = sin(shakeTime * 50) * 10; panel.setPosition(300 + offset, 135);
                if (shakeTime >= 0.4f) { 
                    shaking = false; 
                    panel.setPosition(300, 135); 
                } 
    ...
    int handleClick(sf::Vector2f mousePos) {
        if (askRatingVisible) {
            if (yesButton->handleClick(mousePos))
                return 3;
            if (noButton->handleClick(mousePos))
                return 4; 
            return 0; 
        } 
        if (returnButton->handleClick(mousePos)) 
            return 1;
