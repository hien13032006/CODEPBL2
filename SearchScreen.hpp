...
sf::Text titleText, resultText; 
    ...
    const float VIEW_Y = 200.0f;
    const float VIEW_W = 990.0f;
    const float VIEW_H = 540.0f;
...
public:
    SearchScreen(sf::Font& font, LibrarySystem* lib) : libSystem(lib) {
        ...
        resultText.setFillColor(Theme::TextLight);
        resultText.setPosition(280, 160);
    ...
    void performSearch(sf::Font& font) {
        ...
        if (keyword.empty()) {
            ...
            resultText.setFillColor(Theme::Error);
            return;
        }
        if (!libSystem) 
            return; 
        ...
        if (count == 0) {
            resultText.setString("Khong tim thay ket qua nao.");
            resultText.setFillColor(Theme::Error); 
        } 
        else 
        ...
        scrollView->setMaxScroll(std::max(0.0f, totalH - VIEW_H)); 
        scrollView->reset();
    }
