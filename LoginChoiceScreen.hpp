...
public:
    LoginChoiceScreen(sf::Font& f, Modal* modalRef) : font(f), modal(modalRef), btnClose({1000, 130}, {50, 50}, "X", f, 0, Theme::Error), closed(false) {
        ...
        if (bgTexture.loadFromFile("picChoice1.png")) { bgSprite.setTexture(bgTexture); float scaleY = 500.0f / bgTexture.getSize().y; bgSprite.setScale(scaleY, scaleY); bgSprite.setPosition(250, 110); }
