...
private:
    ...
    Button *librarianButton, *readerButton, btnClose;
...
public:
    LoginChoiceScreen(sf::Font& f, Modal* modalRef) : font(f), modal(modalRef), btnClose({1020, 120}, {50, 50}, "X", f, 0, Theme::Error), closed(false) {
        ...
        descText.setFillColor(Theme::TextLight); 
        descText.setPosition(680, 240);
        librarianButton = new Button({700, 300}, {250, 60}, "Thu Thu", font, 0, Theme::Primary);
        readerButton = new Button({700, 380}, {250, 60}, "Doc Gia", font, 0, Theme::Secondary);
    }
    ~LoginChoiceScreen() {
    ...
    void handleEvent(sf::Event& e, const sf::Vector2f& mousePos) {
        if (e.type == sf::Event::MouseButtonPressed && btnClose.handleClick(mousePos)) 
            closed = true; 
    }
    ...
    int handleClick(sf::Vector2f mousePos) { 
        if (librarianButton->handleClick(mousePos)) 
            return 1; 
        if (readerButton->handleClick(mousePos)) 
            return 2; 
        return 0; 
    }
