...
private:
    ...
    ScrollView *scrollTop10, *scrollAll; 
    LibrarySystem* libSystem; 
    bool isSearchTriggered;
    
    const float VIEW_X = 250.0f; 
    const float TOP_Y = 130.0f; 
    const float TOP_H = 280.0f; 
    const float ALL_Y = 460.0f; 
    const float ALL_H = 280.0f; 
    const float VIEW_W = 1050.0f;
...
public:
    HomeScreen(sf::Font& font, LibrarySystem* lib) : libSystem(lib) {
        ...
        titleTop10.setFont(font); titleTop10.setString("TOP 10 SACH HAY NHAT"); 
        titleTop10.setCharacterSize(20); titleTop10.setFillColor(Theme::TextDark); 
        titleTop10.setPosition(280, 90);
        
        titleAll.setFont(font); titleAll.setString("KHAM PHA THEM"); 
        titleAll.setCharacterSize(20); titleAll.setFillColor(Theme::TextDark); 
        titleAll.setPosition(280, 420); 
    ...
    void loadBooks(sf::Font& font) {
        ...
        for (int i = 0; i < topCount; ++i) {
            std::string id = libSystem->getTop10ID(i); // Lấy ID từ mảng
    ...
    bool checkSearchClick(sf::Vector2f mousePos) { 
        if (searchBtnIcon->handleClick(mousePos) && !searchBox->getText().empty()) { 
    ...
    void render(sf::RenderWindow& window) {
        ...
        // Render Top 10 Viewport
        sf::View viewTop = window.getDefaultView(); 
        viewTop.setViewport(sf::FloatRect(VIEW_X/1300.f, TOP_Y/720.f, VIEW_W/1300.f, TOP_H/720.f)); 
        ...
        viewAll.setViewport(sf::FloatRect(VIEW_X/1300.f, ALL_Y/720.f, VIEW_W/1300.f, ALL_H/720.f)); 
