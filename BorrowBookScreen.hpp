...
private:
    ...
    sf::Text titleText, bookInfoText, borrowMsg, errMaSach; 
    InputField* maSachField; Button *borrowButton, *cancelButton; 
    Sach* currentBook; 
    bool shaking = false; 
...
public:
    BorrowBookScreen(sf::Font& font, Modal* modalRef) : modal(modalRef), currentBook(nullptr) {
        ...
        errMaSach.setFillColor(Theme::Error); 
        errMaSach.setPosition(340, 385);
        
        // borrowMsg dùng chung cho cả lỗi và thành công
        borrowMsg.setFont(font); 
        borrowMsg.setCharacterSize(18); 
        borrowMsg.setPosition(340, 415);
        
        borrowButton = new Button({380, 530}, {200, 55}, "Xac Nhan", font, 0, Theme::Success); 
        cancelButton = new Button({720, 530}, {200, 55}, "Huy", font, 0, sf::Color(150, 150, 150));
    }
    ~BorrowBookScreen() { delete borrowButton; delete cancelButton; delete maSachField; }
    
    void setCurrentBook(Sach* book) {
        currentBook = book; if (currentBook) { 
    ...
    void update(sf::Vector2f mousePos) { 
        ...
        if (shaking) { 
            ...
            panel.setPosition(300 + offset, 110); 
            if (shakeTime >= 0.4f) { 
                shaking = false; panel.setPosition(300, 110); 
            } 
        } 
    }
    ...
    int handleClick(sf::Vector2f mousePos) { 
        if (borrowButton->handleClick(mousePos)) 
            return 1; 
