...
public:
    BorrowBookScreen(sf::Font& font, Modal* modalRef) : modal(modalRef), currentBook(nullptr) {
        ...
        borrowMsg.setFont(font); borrowMsg.setCharacterSize(18); borrowMsg.setFillColor(Theme::Success); borrowMsg.setPosition(340, 415);
    ...
    std::string getMaSachInput() const { return maSachField->getText(); } void setBorrowMessage(const std::string& msg) { borrowMsg.setString(msg); } void startShake() { shaking = true; shakeTime = 0; }
    bool validate() { errMaSach.setString(""); if (getMaSachInput().empty()) { errMaSach.setString("Vui long nhap ma sach!"); startShake(); return false; } return true; }
    ...
    void clearInput() { errMaSach.setString(""); } void clearMessage() { borrowMsg.setString(""); }
};
#endif